...
router.post("/addExploreTodo", (req,res,next)=>{
    ...
    db.query(selectUserQuery, [email]).then((results)=>{
        const uid = results[0].id;
        const insertExploreTodoQuery = `INSERT INTO culture (uid, placename, type, note, todo, favorite, reviewed, location)
        VALUES($1, $2, $3, $4, $5, $6, $7, $8);
        `
...
router.post("/addExploreFavorite", (req,res,next)=>{
    ...
    db.query(selectUserQuery, [email]).then((results)=>{
        const uid = results[0].id;
        const insertExploreFavoriteQuery = `INSERT INTO culture (uid, placename, type, note, todo, favorite, reviewed, location)
        VALUES($1, $2, $3, $4, $5, $6, $7, $8);
        `
